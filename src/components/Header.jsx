...
const Header = () => {
  ...
  {
    label: (
      <p className="m-0 text-[rgba(13,18,22,0.7)] text-[14px] font-semibold pt-1.5 pl-3">Đề xuất</p>
    ),
    key: '2',
  },
  {
    ...
    <div className="fixed w-full z-50 flex justify-between h-[--header-height] px-6 shadow-xl bg-white">
      ...
      <div className="action flex justify-center items-center">
        ...
        <Dropdown
          ...
          arrow={true}
          dropdownRender={() => (
            <div style={{ maxHeight: '400px', background: 'white', overflowY: 'overlay', borderRadius: '10px', scrollbarWidth: 'thin', scrollbars: 'false' }}>
              {items.map((item) => (
                <div key={item.key}>
                  {item.label}
                </div>
