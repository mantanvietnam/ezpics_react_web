...
export default function ProductInfoPrinted(props) {
  ...
  const showModal = async () => {
    ...
    const dataPrint = dataLayer.filter((layer) => {
      return (
        layer?.content?.variable !== "" && layer?.content?.variableLabel !== ""
      );
    });
    if (dataPrint) {
  ...
  return (
    <div className="flex flex-col xl:flex-row w-full h-full mt-[100px] gap-8 px-4">
      <div className="xl:w-[50%] w-full h-full flex flex-col items-center justify-center gap-8">
        ...
        <div className="flex items-center w-[100%] justify-around">
          ...
          {isLoading ? (
            ...
            <div className="flex items-center justify-center gap-1 pl-1 border-l-2 border-slate-300">
              <svg
                width="32"
                height="32"
                viewBox="0 0 24 24"
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col justify-between w-full h-full gap-3 xl:w-1/2">
          <div className="flex items-center gap-3">
            <img src="/images/crown.svg" alt="" />
            <h1 className="text-xl font-semibold">{data?.name}</h1>
          </div>
          <div className="flex items-center gap-5 p-5 bg-slate-100">
            <div className="text-3xl text-red-500">
              {data?.sale_price ? VND.format(data?.sale_price) : "Miễn Phí"}
            </div>
            <div className="line-through rounded-sm text-slate-400">
              {data?.price ? VND.format(data?.sale_price) : ""}
            </div>
            <div className="p-2 font-semibold text-white bg-red-500 rounded-sm">
              {data?.sale_price
              ...
              {!checkModalContent ? (
                      ...
                      dataFilter.map((data, index) => (
                        <div key={index} style={{ marginBottom: "20px" }}>
                          ...
                          {data.content.type === "text" ? (
                            <input
                              value={
                                inputValues[data.content.variableLabel] || ""
                              }
                ...
                <>
                  <div>
                    ...
                    <img
                      src={`data:image/png;base64,${previewsImageAfter}`}
                      alt=""
                    />
                  </div>
                </>
