"use client";
import { detailProductSeriesAPI, getProductAllCategoryAPI } from '@/api/product';
...
export default function Page({ params }) {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            setIsLoading(true)
            try {
                const response = await detailProductSeriesAPI({ idProduct: `${params.product_id}` })
                setData(response?.data?.product)
                setdataLayer(response?.data?.listLayer)
                if (response.data && response.data?.product?.user_id) {
                    try {
                        const userResponse = await getUserInfoApi({ idUser: `${response?.data?.product?.user_id}` });
